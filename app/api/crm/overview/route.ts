    ...
    const data = await tenantTransaction(
      ...
      async (sql) => {
        const [companies, contacts, opportunities, tasks, activities] =
          await Promise.all([
            sql`select id, name, website, industry, size_label as "sizeLabel", status, updated_at as "updatedAt" from companies where organization_id = ${session.organizationId} order by updated_at desc limit 100`,
